...
int main(int argc, char* argv[]) {
    ...
    if (paths.size() < 2) {
        utils::output::writeln_err("mv: missing file operand");
        return 1;
    }
    ...
    if (paths.size() > 1 && !utils::fsutil::is_dir(dest)) {
        utils::output::writeln_err("mv: target '" + dest_raw + "' is not a directory");
        return 1;
    }
    ...
    for (const auto& src_raw : paths) {
        ...
        if (!utils::fsutil::exists(src)) {
            utils::output::writeln_err("mv: cannot stat '" + src_raw +
